...
function createFood() {
    randomfoodX = Math.floor(Math.random() * (mapWidth / foodContainerSize - 1)) * foodContainerSize;
    randomfoodY = Math.floor(Math.random() * (mapHeigth / foodContainerSize - 1)) * foodContainerSize;
    let isInsideSnake = snakeInfos.some( element => element.offsetX === randomfoodX && element.offsetY === randomfoodY);

    // If the food position matches the snake position, than create new positions
    while (isInsideSnake) {
        randomfoodX = Math.floor(Math.random() * (mapWidth / foodContainerSize - 1)) * foodContainerSize;
        randomfoodY = Math.floor(Math.random() * (mapHeigth / foodContainerSize - 1)) * foodContainerSize;
        isInsideSnake = snakeInfos.some( element => element.offsetX === randomfoodX && element.offsetY === randomfoodY);
    }  
...
function testFoodHit() {
    return randomfoodX === snakeInfos[0].offsetX && randomfoodY === snakeInfos[0].offsetY;
}

function testWallHit() {
    return snakeInfos[0].offsetX < 0 || snakeInfos[0].offsetX >= mapWidth || snakeInfos[0].offsetY < 0 || snakeInfos[0].offsetY >= mapHeigth;
}

function testBodyHit() {
   return snakeInfos.some( (element, index) => index > 0 && element.offsetX === snakeInfos[0].offsetX && element.offsetY === snakeInfos[0].offsetY);
}
...
function getResult(isNewRecord) {
    document.body.innerHTML = `<div class="message-container">
        <div class="message-box">
            ...
            <h1 class="score"><i>${points}</i> Pts.</h1>
